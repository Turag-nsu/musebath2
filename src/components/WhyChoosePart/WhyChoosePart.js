...
const WhyChoosePart = () => {
    ...
    return (
        <div className='why-choose-area' >
            <Container>
                <h1 className="title-line">Why Choose our services</h1>
                <div className="cardArea">
                    <Row xs={1} md={4}>
                        {card1Data.map((data, index) =>
                            <Col className='why-choose-area-card-col'>
                                <Card1 key={index} imageLink={data.imageLink} title={data.title} text={data.text} id={id++} />
                            </Col>
                        )}
                    </Row>
